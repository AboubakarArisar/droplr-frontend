...
export const Zone = () => {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setNearbyFiles(prevFiles => 
        prevFiles.map(file => ({
          ...file,
          remainingTime: Math.max(0, EXPIRY_TIME - (Date.now() - new Date(file.createdAt).getTime()))
        }))
      );
    }, 1000);
  ...
  return (
    <div className='min-h-screen bg-gradient-to-br from-[#0f172a] to-[#1e293b] text-white px-4 sm:px-6 py-6 sm:py-10'>
      <div className='max-w-7xl mx-auto'>
        <div className='grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-8'>
          <div className='bg-[#1e293b] border border-gray-700 rounded-2xl p-4 sm:p-8 shadow-lg transition-all duration-300 hover:shadow-xl'>
            <div className='flex flex-col sm:flex-row sm:items-center justify-between mb-4 sm:mb-6 gap-2'>
              <h2 className='text-xl sm:text-2xl font-bold tracking-wide'>
                Drop a File
              </h2>
              <div className='text-blue-400 text-sm'>
                <span className='font-medium'>Location:</span> {latitude?.toFixed(4)}, {longitude?.toFixed(4)}
              </div>
            </div>
            ...
            {loadingFiles ? (
              ...
              <div className='space-y-3 sm:space-y-4 max-h-[400px] sm:max-h-[600px] overflow-y-auto pr-2 custom-scrollbar'>
                {nearbyFiles.map((file, idx) => (
                  <div
                    key={idx}
                    className='group bg-[#0f172a] rounded-xl border border-gray-700 p-3 sm:p-4 transition-all duration-300 hover:border-blue-500/50 hover:shadow-lg hover:shadow-blue-500/10'
                  >
                    <div className='flex items-center justify-between gap-2 sm:gap-3'>
                      ...
                      <button
                        ...
                        disabled={downloadingFile === file.filename || file.remainingTime <= 0}
